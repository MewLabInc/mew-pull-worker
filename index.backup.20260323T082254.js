...
async function claimStagedMenuExtraction() {
  const { data: rows, error } = await supabase
    ...
    .select("id, place_id, candidate_source_id, source_url, provider, raw, status, created_at")
    .eq("status", "staged")
    .order("created_at", { ascending: true })
    .limit(1);
  ...
  const { data: claimedRows, error: claimErr } = await supabase
    ...
    .eq("id", row.id)
    .eq("status", "staged")
    .select("id, place_id, candidate_source_id, source_url, provider, raw, status, created_at, extracted_at")
    .limit(1);
...
function normalizeUrl(raw, baseUrl = null) {
  const s = String(raw || "").trim();
  if (!s) return null;
...
function extractAnchorTags(html, baseUrl = null) {
  const raw = String(html || "");
  const out = [];
  ...
  while ((m = re.exec(raw)) !== null) {
    ...
    const textRaw = String(m[2] || "")
      .replace(/<[^>]+>/g, " ")
      .replace(/\s+/g, " ")
      .trim();

    const href = normalizeUrl(hrefRaw, baseUrl) || hrefRaw;
...
function extractPhonesFromHtml(html) {
  ...
  while ((m = telRe.exec(raw)) !== null) {
    ...
    if (phone) found.set(`tel:${phone}`, { value: phone, confidence: 0.98 });
  }

  const text = stripHtmlToText(raw);
  const phoneTextRe = /(?:\+?\d[\d\s().-]{7,}\d)/g;
  const matches = text.match(phoneTextRe) || [];
  for (const v of matches) {
    const phone = normalizePhone(v);
    if (phone) found.set(`txt:${phone}`, { value: phone, confidence: 0.75 });
  }
...
function extractEmailsFromHtml(html) {
  ...
  while ((m = mailtoRe.exec(raw)) !== null) {
    const email = normalizeEmail(String(m[1]).split("?")[0]);
    if (email) found.set(`mailto:${email}`, { value: email, confidence: 0.98 });
  }

  const text = stripHtmlToText(raw);
  const emailTextRe = /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi;
  const matches = text.match(emailTextRe) || [];
  for (const v of matches) {
    const email = normalizeEmail(v);
    if (email) found.set(`txt:${email}`, { value: email, confidence: 0.8 });
  }

  return Array.from(found.values());
}

function classifyLinkContactType(link) {
  const href = String(link?.href || "").toLowerCase();
  const text = String(link?.text || "").toLowerCase();
  const combined = `${href} ${text}`;

  if (href.startsWith("mailto:")) return { type: "email", confidence: 0.98 };
  if (href.startsWith("tel:")) return { type: "phone", confidence: 0.98 };

  if (/contact|get in touch/.test(combined)) {
    return { type: "contact_url", confidence: 0.92 };
  }

  if (/reserve|reservation|book|opentable|resy/.test(combined)) {
    return { type: "booking_url", confidence: 0.9 };
  }

  if (/order|delivery|takeaway|pickup|collection|ubereats|deliveroo|justeat|doordash|glovo|wolt/.test(combined)) {
    if (/ubereats|deliveroo|justeat|doordash|glovo|wolt/.test(combined)) {
      return { type: "delivery_url", confidence: 0.94 };
    }
    return { type: "order_url", confidence: 0.88 };
  }

  return null;
}

function buildSidecarContactPoints({ html, baseUrl, sourceUrl, sourceDomain, provider }) {
  const points = [];
  const dedupe = new Set();

  const pushPoint = (contact_type, contact_value, confidence) => {
    const value = String(contact_value || "").trim();
    if (!value) return;

    const key = `${contact_type}::${value.toLowerCase()}`;
  ...
  for (const p of extractPhonesFromHtml(html)) {
    pushPoint("phone", p.value, p.confidence);
  }

  for (const e of extractEmailsFromHtml(html)) {
    pushPoint("email", e.value, e.confidence);
  }

  const links = extractAnchorTags(html, baseUrl);
  for (const link of links) {
    const classified = classifyLinkContactType(link);
    if (!classified) continue;

    const normalized = normalizeUrl(link.href, baseUrl);
    if (!normalized) continue;

    pushPoint(classified.type, normalized, classified.confidence);
  }

  return points;
}

async function insertPlaceContactPoints(placeId, contactPoints) {
  if (!Array.isArray(contactPoints) || contactPoints.length === 0) {
    return { inserted: 0 };
  }

  const rows = contactPoints.map((row) => ({
    place_id: placeId,
    contact_type: row.contact_type,
    contact_value: row.contact_value,
    provider: row.provider || "webhunter",
  ...
  if (error) {
    const e = new Error(`ENVELOPE:insert_place_contact_points_failed:${error.message || String(error)}`);
    e.status = 400;
    throw e;
  }
...
"menu.extract.requested": async ({ envelope, meta }) => {
  ...
  if (isPdf) {
    ...
    if (crawlResult?.raw_html) {
      ...
      try {
        const fetched = await fetchMenuSourcePage(effectiveUrl);
        ...
        lines = fetched.raw_html ? htmlToLines(fetched.raw_html) : textToLines(fetched.raw_text);
      } catch (err) {
