...
"menu.extract.requested": async ({ envelope, meta }) => {
  ...
  await updateMenuExtractionRow(extraction.id, {
    raw,
    item_count: raw.menu.sections?.[0]?.items?.length || 0,
    extractor: "webhunter_v1",
    extractor_version: "v1_basic",
...
await supabase.from("menu_extractions").insert({
  ...
  source_url: fetched.detected_menu_url || fetched.final_url || fetched.source_url,
  provider: "webhunter",
  ...
  raw: {
    source: {
      ...
      url: fetched.detected_menu_url || fetched.final_url || fetched.source_url
    },
    meta: {
      detected_at: new Date().toISOString(),
